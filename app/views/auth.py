...
no_auth_required_routes = ['login', 'register', 'static','register_admin']
    ...
    try:
        ...
        if form.validate_on_submit():
            try:
                ...
                new_secret = Secret(value=secret.get("value"), key=secret.get("key"))
                new_secret.save()
...
@app.route('/edit_secret', methods=['GET', 'POST'])
@admin_required
def edit_secret():
    try:
        form = SecretForm(request.form)
        ...
        if form.validate_on_submit():
            try:
                ...
                if Secret.if_exists(id=secret_id):
                    Secret.update(id=secret_id, new_value=secret_data.get("value"), new_key=secret_data.get("key"))
                ...
                flash(ErrorMessages.SAVE_SECRET.value, "error")
                return redirect(url_for('get_secrets'))
        return render_template('home/secret.html', form=form, secret_id=secret_id)
    except Exception:
        logging.warning(str(traceback.format_exc()))
        flash(ErrorMessages.GET_SECRET.value, "error")
        return redirect(url_for('get_secrets'))
