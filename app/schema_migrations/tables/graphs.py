...
class GraphsViewPanelToString(BaseMigration):
    name = "graphs: view_panel INTEGER -> VARCHAR(50)"
    ...
    def apply(self, connection, inspector):
        ...
        if 'view_panel' not in graphs_cols_by_name:
            return

        col_type = str(graphs_cols_by_name['view_panel'].get('type', '')).upper()
        if 'INT' not in col_type:
            return  # already migrated

        log.info(f"Applying migration for '{graphs_table}': view_panel INTEGER -> VARCHAR(50)")
        ...
        try:
            ...
            connection.execute(create_sql)
            connection.execute(text(
                f"INSERT INTO graphs_new (id, project_id, name, type, grafana_id, dash_id, view_panel, width, height, custom_vars, prompt_id) "
                f"SELECT id, project_id, name, type, grafana_id, dash_id, CAST(view_panel AS TEXT), width, height, custom_vars, prompt_id FROM {graphs_table}"
            ))
            connection.execute(text(f'DROP TABLE {graphs_table}'))
            connection.execute(text('ALTER TABLE graphs_new RENAME TO graphs'))
            connection.execute(text('CREATE INDEX IF NOT EXISTS ix_graphs_project_id ON graphs (project_id)'))
            ...
            try:
                ...
                pass
        log.info("Migration for 'graphs' view_panel completed via SQLite table rebuild.")


# Export list of migrations for this table
MIGRATIONS = [
    GraphsAddTypeRelaxNulls(),
    GraphsViewPanelToString(),
]
