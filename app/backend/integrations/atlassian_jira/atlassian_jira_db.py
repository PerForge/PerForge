...
class DBAtlassianJira(db.Model):
    ...
    jira_project_key = db.Column(db.String(120), nullable=False)
    epic_field       = db.Column(db.String(120))
    epic_name        = db.Column(db.String(120))
    ...
    def save(cls, project_id, data):
        try:
            ...
            validated_data = AtlassianJiraModel(**data)
    ...
    def get_configs(cls, project_id):
        try:
            ...
            return [AtlassianJiraModel(**config).model_dump() for config in configs]
        except Exception:
            logging.warning(str(traceback.format_exc()))
            raise
